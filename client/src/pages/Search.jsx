...
export default function Search() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      setSidebardata({
        // Setting sidebardata state with values from URL or defaults
  ...
  const handleChange = (e) => {
    // Defining handleChange function for form inputs
    if (
      e.target.id === "all" ||
      e.target.id === "rent" ||
      e.target.id === "sale"
    ...
    if (
      ...
      setSidebardata({
        ...sidebardata,
        [e.target.id]:
          e.target.checked || e.target.checked === "true" ? true : false, // Updating boolean values in sidebardata state
      });
    }
  ...
  const onShowMoreClick = async () => {
    // Defining onShowMoreClick function to fetch more listings
    ...
    const data = await res.json(); // Parsing response as JSON
    if (data.length < 9) {
      // Checking if less than 9 listings are returned
      setShowMore(false); // Setting showMore state to false
    }
  ...
  return (
    <div className="flex flex-col justify-center md:flex-row px-3 max-w-6xl mx-auto py-6">
      <div className="border-b-2 md:border-r-2 md:min-h-screen pr-10">
      ...
      <div className="flex-1">
        <h1 className="text-3xl font-semibold border-b p-3 text-slate-700 mt-5">
          Header for listings results:
        </h1>
        {/* Container for listing items */}
        <div className="py-7 px-3 flex flex-wrap gap-4">
          {!loading &&
          listings.length === 0 && ( // Message if no listings found
          ...
          {!loading &&
            listings &&
            listings.map((
              listing // Mapping over listings to display ListingItem components
